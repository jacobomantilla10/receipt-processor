...
def process():
    ...
    try:
        ...
        if 'items' in request_data:
            ...
            return 'Req JSON missing items', 400

        if 'total' in request_data:
            receipt_object['total'] = float(request_data['total'])
        else:
            return 'Req JSON missing total', 400
    except:
        return 'Req body empty or field not formatted correctly', 400
